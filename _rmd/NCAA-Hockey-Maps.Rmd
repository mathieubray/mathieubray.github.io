...
![](/Users/braym/Desktop/Personal Files/Website/mathieubray.github.io/plots/map-screenshot.jpg)
...
players <- read.csv("C:/Users/braym/Box Sync/Personal/Hockey/NCAA/Projects/Maps/NCAAPlayersRaw.csv",header=T,stringsAsFactors=F)
...
players <- read.csv("C:/Users/braym/Box Sync/Personal/Hockey/NCAA/Projects/Maps/PlayersHometown.csv",header=T,stringsAsFactors=F)
...
logo.images <- read.csv("C:/Users/braym/Box Sync/Personal/Hockey/Logos/LogoInfo.csv",header=T,stringsAsFactors=F) %>%
  filter(League == "NCAA") %>%
  mutate(Team = ifelse(Team == "Lake Superior","Lake Superior State",Team),
         Image = paste0("C:/Users/braym/Box Sync/Personal/Hockey/Logos/NCAA/",FileName,".png"),
...
players <- read.csv("C:/Users/braym/Box Sync/Personal/Hockey/NCAA/Projects/Maps/PlayersHometown.csv",header=T,stringsAsFactors=F) %>%
  ...
  mutate(Lon = Lon + runif(1,min=-0.025,max=0.025), # Add some jitter to coordinates 
         Lat = Lat + runif(1,min=-0.025,max=0.025)) # (so that players in one city aren't stacked)
```
...
# We'll filter our data set so that only the Big Ten teams appear in the plot
teams <- c("Michigan","Michigan State","Ohio State","Penn State","Minnesota","Wisconsin") 

big.ten <- players %>% filter(Team %in% teams)
...
get.file <- function(team){ # Retrieve image file location
