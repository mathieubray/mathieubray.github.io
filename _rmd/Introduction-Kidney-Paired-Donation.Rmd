...
```{r kpd_1,warning=F,echo=F,message=F}
...
example1 <- matrix(c(2,1),nrow=1)

graph1 <- graph_from_edgelist(example1)

V(graph1)$Pair = c("1","1")
V(graph1)$Profile = c("A","B")
E(graph1)$Compatible = "No"

ggraph(graph1, layout = 'kk') +
  geom_edge_link(aes(edge_linetype=Compatible,
                     start_cap=circle(18,"mm"),
                     end_cap=circle(18,"mm")),
                 arrow=arrow(angle=20, 
                             length = unit(0.1,"inches"),
                             type="closed")) +
  geom_node_point(aes(color=Pair,shape=Profile),size=32,alpha=0.6,show.legend=FALSE) +
  geom_node_text(aes(label=c("Candidate","Donor")),size=4, color="black") +
  scale_color_manual(values=c("red")) +
  scale_edge_linetype_manual(values=c("dashed")) +
  ylim(-1000,1000) + xlim(-1.05,-0.85) + theme_graph() +
  ggtitle("Willing Donor is Incompatible with Intended Recipient",
          subtitle="Shapes abstract the blood type/antigen profiles of the donor and candidate\nThe differing shapes here represent a mismatch")
...
```{r kpd_2,warning=F,echo=F,message=F}

example2 <- matrix(c(1,2,3,4,1,4,3,2), nrow=4, byrow=TRUE)

graph2 <- graph_from_edgelist(example2)

V(graph2)$Pair = c("1","1","2","2")
V(graph2)$Profile = c("A","B","B","A")
E(graph2)$Compatible = c("No","No","Yes","Yes")
...
ggraph(graph2, layout = 'kk') +
  geom_edge_link(aes(edge_linetype=Compatible,
                 ...
                 arrow=arrow(angle=20, 
                             length = unit(0.1,"inches"),
                             type="closed")) +
  geom_node_point(aes(color=Pair, shape=Profile),size=26,alpha=0.6) +
  geom_node_text(aes(label=c("Donor 1","Candidate 1","Donor 2","Candidate 2")),size=3, color="black") +
  ...
  ylim(-1.25,1.25) + xlim(-1.25,1.25) + theme_graph() +
  ggtitle("Both Patients Receive Transplants",
          subtitle="Donor 1 matches with Candidate 2; Donor 2 matches with Candidate 1\nA simple swap of donors allows for two otherwise unobtainable transplants")
